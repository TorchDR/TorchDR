...
def silhouette_samples(
        X, labels, weights=None, metric='euclidean'):
    """    
    ...
    """
    # to do: keops compatibility + numpy ?
    def compute_distances(X1, X2):  # check for metrics used in torchdr
        if metric == 'euclidean':
            return torch.cdist(X1, X2, p=2)
        else:
            raise ValueError(
                f'unknown metric = {metric}, pick one in ["euclidean"]')
    ...
    for i, pos_i in enumerate(pos_labels[:-1]):
        intra_cluster_dists = compute_distances(X[pos_i], X[pos_i])
        ...
        for pos_j in pos_labels[i + 1 :]:
            inter_cluster_dists = compute_distances(X[pos_i], X[pos_j])
...
def silhouette_score(X, labels, weights=None, metric='euclidean',
    ...
    """
    # to do: keops + batch sampling (+ numpy ?)
    coefficients = silhouette_samples(X, labels, weights, metric)
    silhouette_score = torch.mean(coefficients)
