...
class SyntheticDataset(torch.utils.data.Dataset):
    ...
    def get_children(self, parent_value, parent_label, current_depth, offspring=True):
        ...
        if offspring:
            ...
            child_value_1 = self.ball.geodesic_unit(torch.tensor(sigma), parent_value, direction)
            child_value_2 = self.ball.geodesic_unit(torch.tensor(sigma), parent_value, -direction)
            ...
            for i in range (numberOfChildren):
                child_value = self.ball.random(self.dim, mean=parent_value, std=sigma ** .5)
...
dataset = SyntheticDataset(ball, 50, 2, numberOfsiblings=100, sigma_sibling=0.05, dist_children=.7)
